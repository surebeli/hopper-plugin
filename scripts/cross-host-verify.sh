...
# true regardless of whether you run all 4 hosts simultaneously:
#
#   1. All 4 host entry points reference the SAME hopper-dispatch binary
...
# ─── Check 1: all 4 host entry points exist ────────────────────────────
...
for f in "$TIER_A" "$TIER_B_MANIFEST" "$TIER_C1" "$TIER_C2"; do
  if [ -f "$f" ]; then
    pass "$f"
  else
    fail "missing: $f"
...
for f in "$TIER_B_CMD_DIR/dispatch.md" "$TIER_C1" "$TIER_C2"; do
  base=$(basename "$f")
