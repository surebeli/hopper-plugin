...
| inventory/probe | 1–7 | inventory cache version 不支持 → `inventory-cache-version-unsupported`；inventory cache 畸形 → `inventory-cache-malformed`；recovery atomic replace 在 commit point 前失败 → `inventory-cache-recovery-replace-failed`；atomic replace 成功后 durability 未确认 → `inventory-cache-recovery-durability-unknown`；capabilities command 失败 → `capability-failed`；probe spawn/exit/parse failure → `probe-failed`；无安全 catalog → `catalog-unavailable`。 |

当多个 domain 同时报告时，最终单一 code 的跨 domain 优先级是：`runtime-model-metadata-malformed`、`runtime-model-metadata-conflict`、`metadata-envelope-malformed`、`selector-metadata-cache-schema-unsupported`、`selector-metadata-cache-adapter-mismatch`、`selector-metadata-cache-expired`、`selector-metadata-cache-missing`、`inventory-cache-version-unsupported`、`inventory-cache-malformed`、`inventory-cache-recovery-replace-failed`、`inventory-cache-recovery-durability-unknown`、`capability-failed`、`probe-failed`、`catalog-unavailable`、`runtime-model-metadata-absent`、`unknown`。这使 runtime malformed 必定先于 conflict，schema unsupported 必定先于 adapter binding mismatch，并使尚未跨过 commit point 的 replace failure 与已提交后 durability unknown 保持可区分，同时保留各 domain 的可实施触发条件。
...
| `alias`/`auto` 带 expected identity，或 `concrete` 的 kind/member/provider 不合该 vendor contract | 任意 | `unverified` | envelope 必须报 `metadata-envelope-malformed`、selector 归一为 `unknown`；不能以 runtime evidence 弥补无效 metadata。 |
...
显式 recovery surface 是既有 `hopper-dispatch --probe <vendor>` 加新增、必须实现并测试的 `--recover-cache` flag：只有用户执行 `hopper-dispatch --probe <vendor> --recover-cache` 时，工具才可恢复。流程的唯一 commit point 是 atomic replace active cache；以下所有可失败准备必须在此之前完成：(1) inspect 原 active cache；(2) 生成并完整 validate fresh v1 temp；(3) 若 active 存在，创建同目录受控 backup，命名固定为 `<cache-basename>.recovery-<UTC-compact>-<8-lowercase-hex>.bak`，设置并**重新读取/assert** owner-only 权限（Windows 使用当前用户 ACL 等价物）；ACL assertion 失败即 abort；若 active 缺失则不创建 backup，但仍必须 temp validate；(4) 计算并执行固定上限 3 个 recovery backups 的 retention/prune。retention 按 parsed UTC timestamp 由旧到新排序；相同 timestamp 必须以完整 backup basename 的 ordinal bytewise order 作为稳定 tie-breaker，再删除最旧项以保留最多 3 个。prune 必须在 replace 前完成：若 prune 已成功而随后在 replace 前 crash/fail，active 仍 byte-for-byte unchanged，已删 backup 不被“rollback”恢复，temp 仍只 best-effort cleanup，已创建且未被 prune 的 backup 可以保留。任一其余 pre-commit step 失败时 active cache 同样必须 byte-for-byte unchanged，temp cleanup 仅 best-effort，已经创建的 backup 可以保留。只有准备全部成功后才 atomic replace；若该 atomic replace 在 commit point 前报告失败，返回 closed diagnostic `inventory-cache-recovery-replace-failed`，active 必须 byte-for-byte unchanged，并适用相同 backup/temp cleanup 规则。replace 后不得进行 retention/prune 或声称能 rollback。
...
实施 landing sequence 是强制顺序，不是扩大本协议的额外 lifecycle feature：先冻结 vendor-discriminated types/metadata 与 strict comparator contract，再让 parser-to-finalizer 数据流收口，然后写 file-backed protocol/readers；其后先完成 cache recovery transaction（含 pre-commit replace-failure、post-commit durability、prune-before-crash 与 ACL fixtures），再接 cache public projection 与 dashboard v2 safe shim，并在每步完成对应 fixture/compat 断言后进入下一步。共享 lifecycle 文件的阶段边界、revalidation、idle/terminal sequencing 仅作为实施检查项：必须确认新字段没有改变一次 spawn、唯一 terminal event、resume/revalidation 或 timeout 行为，但本设计不新增 lifecycle phase、状态转换或跨任务协议。

### Phase 1：合同与数据路径

1. **Types、adapter metadata、比较边界：** 在 `cli/src/types.js` 的 adapter/result contract 加 raw/effective selector provenance、`alias|concrete|auto|unknown` selector metadata 与 optional `modelAttestation`；实现 versioned selector-metadata envelope/schema、binding validator 和每个 selector record 的 exact-literal vendor-discriminated union（alias/auto 无 expected identity；Claude concrete 仅 `{identity_kind:"opaque-id",id}`；OpenCode/已声明 structured adapter concrete 仅 `{identity_kind:"provider-model",provider,model}`，禁止 sentinel/dead provider 与 cross-vendor kind）。无效 identity 必须成为 `metadata-envelope-malformed`/`unverified`。Claude/OpenCode 只从 sanitized/versioned capabilities cache 或 adapter 发布的 versioned manifest 零 spawn 分类，Kimi 仅声明可证实的 exact record。扩展 `cli/src/model-normalize.js` 为唯一 comparison boundary，令 `cli/src/model-check.js` 与 `cli/src/dispatch.js` 调用明确分离的 validation/strict-runtime modes；strict mode 不得调用 `modelKeysMatch`，adapter 不实现第二个 canonicalizer。
...
| Claude/OpenCode zero-spawn selector metadata | schema v1 cache envelope/manifest、vendor-adapter-catalog binding、validity/time/version、每条完整 selector literal、alias/auto/concrete discriminated record；Claude concrete 仅 `{identity_kind:"opaque-id",id}`，OpenCode concrete 仅 `{identity_kind:"provider-model",provider,model}`；duplicate/conflicting literal、unknown forward field；fixtures 覆盖 `fable`、`sonnet`、`sonnet[1m]`、`best`、`default`、`opusplan`、未枚举 `[N_unit]` | dispatch 不 spawn/probe/network；只对 post-policy effective selector 用有效 envelope 分类，且不建立 alias→actual 映射；unsupported schema/adapter mismatch/过期/缺失/malformed/duplicate/未枚举为 `selector_kind=unknown`、`unverified`，绝不 false `mismatch`/`alias-resolved`。 |
| Expected identity union validation | alias/auto 携带 expected identity；Claude 使用 provider-model；OpenCode/structured 使用 opaque-id；concrete 缺/多 member、unknown kind、sentinel/dead provider 或 non-live structured provider | 每项均为 `metadata-envelope-malformed`、`selector_kind=unknown` 与 `unverified`，不调用 strict comparator，不能由 observed value 修复。 |
...
| Cache/probe semantics | missing cache、旧 v1 entry、含 root/vendor/nested unknown future field 的 v1 entry、version mismatch、malformed cache、ordinary setup/capabilities/probe、`--probe <vendor> --recover-cache` success/failure、backup name/ACL assertion/retention/backup failure、replace failure、完整 `inventoryContractVersion` fixture（version、六个 v2 fields、direct vendor shim）及 missing/unknown/future version variants | diagnostics-aware reader 区分 missing/ok-v1/mismatch/malformed；ordinary surface 只给 closed diagnostic+recover hint且不写；仅 missing 初始化、v1 owned fields additive merge 且所有 unknown field 保留；mismatch/malformed 为 unknown、普通 writer no-write，version mismatch 原文件字节不变；explicit recovery 仅在 owner-only backup ACL assertion 成功后 atomic replace fresh-v1，固定 retention 以 timestamp+basename stable order，任何 backup failure abort且不降格为 missing；replace failure 使用 `inventory-cache-recovery-replace-failed`；完整 `inventoryContractVersion` response 仍只使用 safe projection/shim，version variants 不 throw 或泄露 raw value。 |
...
| Recovery transaction (F2) | existing-cache inspect/temp validate/backup/ACL assertion/retention-prune 任一步失败；equal-timestamp backup names；prune 后、replace 前 crash；atomic replace failure；missing cache；crash after replace；post-commit fsync/durability failure | ACL success 必须已断言；retention 按 timestamp 再 basename tie-break，prune-before-crash 后 active 仍 byte-for-byte unchanged且不恢复已删 backup；所有 pre-commit failure 与 replace failure 都保持 active byte-for-byte unchanged、temp best-effort cleanup、已有 backup 可保留，replace failure 使用 `inventory-cache-recovery-replace-failed`；missing cache 不建 backup仍 temp validate+atomic install；成功 replace 是唯一 commit point，after-crash 只验证新 active、绝不自动 rollback；仅成功 replace 后的 durability failure 使用 `inventory-cache-recovery-durability-unknown` 且承认 active 可能已更新。 |
