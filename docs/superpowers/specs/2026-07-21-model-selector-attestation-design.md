...
| `selector_kind` | effective selector 的 `alias`、`concrete`、`auto` 或 `unknown`。`auto` **只**由 `effective_selector=null` 导出，绝不是 metadata 中的 literal selector record；`unknown` 表示显式 selector 不能由当次 metadata 安全枚举/分类。 | policy/default resolution；非 null selector 才使用 adapter 的 declarative/dynamic selector metadata，而非正则猜测 |
...
原始 selector 永远保留用于审计和显示；policy 若替换、补充或清空 selector，必须同时保留 raw/effective 两值及 source。runtime resolution 一律比较 `effective_selector`，从不比较 `requested_selector`；两者相同也不得因此省略 provenance。`effective_selector=null` 时 resolver 在 metadata lookup 前直接导出 `auto`，不会把 `auto` 当作字符串或记录查找。adapter 只声明可枚举的非 null selector 分类和 runtime result 中可读取的 metadata path；无法枚举/分类的显式值为 `unknown`，而不是默认 `concrete`。adapter 不实现或调用独立的 ID 比较器。
...
2. **runtime attestation：** 使用同模块导出的独立 strict comparator，绝不调用、复用或间接包裹 `modelKeysMatch` 的 namespace strip/tail-match 路径。它只做 trim，以及该 vendor 明确声明为 case-insensitive 的完整 component 的 case folding；不得 strip provider prefix、namespace、分隔符，不得 tail-match、fuzzy-match 或把 alias 展开为 ID。Claude concrete record 只能声明 `{ identity_kind: "opaque-id", id: <full literal> }`；它是 opaque、完整的 Claude ID，observed ID 与 `id` 必须在上述规范化后整体相同。OpenCode（以及 metadata contract 明确声明结构化 provider/model runtime identity 的 adapter）concrete record 只能声明 `{ identity_kind: "provider-model", provider, model }`；两侧都必须可解析为恰好同一 normalized provider component 和同一 normalized model component。bare `model` 只可与 bare `model` 整体相等，永远不能与 `provider/model`、`namespace/provider/model` 或任何带 namespace 的 ID 相等。不得把 `opaque-id` 用于 OpenCode/structured record，也不得把 `provider-model` 用于 Claude；不能以 sentinel、dead/unavailable provider、placeholder 或跨 vendor identity kind 充当 expected identity。缺失/多余/错误 kind 的 expected identity 是 metadata envelope malformed，而不是一项 runtime non-match；额外 namespace、解析歧义或只有 bare/qualified 一侧才是“不可比较”。只有有效 vendor-discriminated strict path 能决定 concrete 的 `exact` 或 `mismatch`；不可比较或无证据只能为 `unverified`。
...
canonical storage field 为 `diagnostic_code`，public renderer 输出同值为 `diagnosticCode`。它的闭集为 `none|metadata-envelope-malformed|selector-metadata-cache-schema-unsupported|selector-metadata-cache-adapter-mismatch|selector-metadata-cache-expired|selector-metadata-cache-missing|runtime-model-metadata-malformed|runtime-model-metadata-conflict|runtime-model-metadata-absent|inventory-cache-version-unsupported|inventory-cache-malformed|inventory-cache-recovery-backup-create-failed|inventory-cache-recovery-replace-failed|inventory-cache-recovery-durability-unknown|capability-failed|probe-failed|catalog-unavailable|unknown`。diagnostic 先在独立 domain 内判定，再按最后的跨 domain 优先级合并为一个 code；raw error 永远不改变选择结果：

| domain | domain 内优先级（从高到低） | 触发条件与 code |
| --- | --- | --- |
| selector-metadata validation | 1–5 | envelope 字段/duplicate/conflicting literal、alias 携带 expected identity、出现 `auto` record、concrete 缺失/多余/跨 vendor kind 的 expected identity，或 provider-model 不在 live registry/落入 sentinel set → `metadata-envelope-malformed`；支持 schema 外 → `selector-metadata-cache-schema-unsupported`；schema 已支持但 vendor/adapter id/version/catalog binding 不符 → `selector-metadata-cache-adapter-mismatch`；时间/validity 无效或过期 → `selector-metadata-cache-expired`；无有效 envelope → `selector-metadata-cache-missing`。 |
...
| inventory/probe | 1–8 | inventory cache version 不支持 → `inventory-cache-version-unsupported`；inventory cache 畸形 → `inventory-cache-malformed`；recovery backup 的 exclusive creation（包括 8 次 fresh candidate 都碰撞）失败 → `inventory-cache-recovery-backup-create-failed`；recovery atomic replace 在 commit point 前失败 → `inventory-cache-recovery-replace-failed`；atomic replace 成功后 durability 未确认 → `inventory-cache-recovery-durability-unknown`；capabilities command 失败 → `capability-failed`；probe spawn/exit/parse failure → `probe-failed`；无安全 catalog → `catalog-unavailable`。 |

当多个 domain 同时报告时，最终单一 code 的跨 domain 优先级是：`runtime-model-metadata-malformed`、`runtime-model-metadata-conflict`、`metadata-envelope-malformed`、`selector-metadata-cache-schema-unsupported`、`selector-metadata-cache-adapter-mismatch`、`selector-metadata-cache-expired`、`selector-metadata-cache-missing`、`inventory-cache-version-unsupported`、`inventory-cache-malformed`、`inventory-cache-recovery-backup-create-failed`、`inventory-cache-recovery-replace-failed`、`inventory-cache-recovery-durability-unknown`、`capability-failed`、`probe-failed`、`catalog-unavailable`、`runtime-model-metadata-absent`、`unknown`。这使 runtime malformed 必定先于 conflict，schema unsupported 必定先于 adapter binding mismatch，backup-name exhaustion 不会被误报为 replace failure，并使尚未跨过 commit point 的 replace failure 与已提交后 durability unknown 保持可区分，同时保留各 domain 的可实施触发条件。

`sourceNote` 是仅供私有受控诊断日志使用的原始来源说明术语，不是 cache、handoff、frontmatter、JSONL、dashboard/API 或 CLI summary 的字段；`notes`、`sourceNote`、`cacheError` 与 `modelsSource` 都不是 public data-bearing fields。唯一例外是后文限定的每个 direct `vendors[]` record 上四个 legacy key 的固定 inert safe shim。raw probe/能力说明只可留在该私有日志，且不得复制进 cache `notes`、frontmatter、JSONL 或任何 public surface；原始 stderr、auth/login 文案、provider 名称、parse exception、config/binary path、URL、environment value 和 credential 均同样禁止。
...
分类输入必须是下列 schema v1 的已 sanitize metadata envelope；schema fields 缺失、类型不符或值不在闭集即拒绝。`schema_version` 当前只支持 `1`，future version 不做 best-effort parse：归为 `selector-metadata-cache-schema-unsupported` 并得到 `unknown`。每条 **非 null** selector record 都是一个完整 exact literal；不存在 selector literal 的拼接、variant 组合、regex 或 grammar。`auto` 没有 literal record：它只由 policy/default resolution 后的 `effective_selector=null` 导出。结构示意如下：
...
```json
{
  ...
  "adapter": {
    ...
    "identity_declaration": {
      "identity_kind": "opaque-id | provider-model",
      "provider_registry": ["exact live provider literal; provider-model only"],
      "provider_sentinels": ["exact non-live provider literal; provider-model only"]
    }
  },
  ...
  "selectors": [
    {
      "literal": "exact alias selector literal",
      "kind": "alias"
    },
    {
      "literal": "claude exact concrete selector literal",
      "kind": "concrete",
      "expected_runtime_identity": {
        "identity_kind": "opaque-id",
        "id": "the same full Claude literal"
      }
    },
    {
      "literal": "opencode exact provider/model selector literal",
      "kind": "concrete",
      "expected_runtime_identity": {
        "identity_kind": "provider-model",
        "provider": "one live exact provider literal",
...
每个 adapter declaration 必须含一个 `identity_kind`；`opaque-id` adapter 的 `provider_registry` 与 `provider_sentinels` 均为空 closed set，`provider-model` adapter 则必须声明两个无重复、互不相交的 exact-literal closed set：live `provider_registry` 与 non-live `provider_sentinels`。只有 registry 中的 provider 可作为 concrete expected identity；sentinel 或任何未列 provider 都无效，不能因 future/provider-like 文本而放宽。record validation 是 vendor-discriminated union：`alias` record **禁止** `expected_runtime_identity`；`auto` record 永远不合法，因为 `auto` 不是 metadata record；`concrete` record **必须且只能**有一个与 adapter declaration 和 envelope vendor 一致的 expected identity。Claude concrete 恰为 `{identity_kind:"opaque-id",id}`，`id` 是 non-empty full literal，不能同时携带 `provider`/`model` 或其他 identity member；OpenCode（或已声明同一 structured runtime schema 的 future adapter）concrete 恰为 `{identity_kind:"provider-model",provider,model}`，两个值均为 non-empty exact literals，不能用 arrays、independent lists 或 cross-product 表达，且 `provider` 必须在该 adapter declaration 的 live structured registry 中，不能在 sentinel set 中或是 dead/unavailable/unknown placeholder。Claude 不得使用 `provider-model`，OpenCode/structured adapter 不得使用 `opaque-id`，任何未知 identity kind、declaration 缺失/不一致、cross-vendor kind、缺失/多余 member、sentinel 或未注册 provider 都使整个 envelope 成为 `metadata-envelope-malformed`，selector 归一为 `unknown`、resolution 为 `unverified`，且绝不进入 strict comparator。相同 `literal` 出现两次（即使内容相同）或同 literal 有冲突 `kind`/expected identity 时，整个 envelope 同样 malformed。`sonnet[1m]`、`fable`、`best` 等都必须各自作为完整 literal 单独列出；未枚举 `sonnet[1m]`、任意 `[N_unit]`、大小写/分隔符变体或未来形式一律不匹配。该 schema 永不包含 alias→actual model 映射。

zero-spawn source priority 是：(1) vendor、adapter id、adapter version、catalog version 都匹配且未过期的 sanitized capabilities-cache envelope；(2) 匹配同一 binding 的 adapter-manifest envelope；(3) 无有效 source。dispatch 只读该两项已有本地数据，绝不为了分类执行 CLI、probe 或网络请求。每个 envelope 必须 `generated_at <= expires_at` 且 `validity=accepted`；vendor/adapter mismatch、schema unsupported、过期/无效时间、缺失或 malformed envelope 均拒绝。cache 对未知 future fields 的读取策略是：已支持 schema 内忽略但不解释 unknown fields，兼容 writer 仅在同 schema additive merge 时保留它们；schema version 不支持时不读、不分类、不覆盖。`effective_selector=null` 在选择 source/record 前已导出 `auto`；只有 non-null effective selector 在两项 source 都无效或完整 selector literal 未枚举时才为 `unknown`/`unverified`，不得产生 `mismatch` 或 `alias-resolved`。
...
| Claude metadata 明确列为完整 model ID，例如 `claude-sonnet-4-6` | `concrete` | record 只能携带 `{identity_kind:"opaque-id",id:<full literal>}`，与 observed actual IDs 做 opaque strict runtime comparison。 |
...
| OpenCode metadata 明确以 `provider/model` complete literal 传入和报告 | `concrete` | record 只能携带 `{identity_kind:"provider-model",provider,model}`，与 record 的唯一 expected provider/model pair 严格比较。 |
| Kimi config alias | `alias` | 若 runtime result 没有可用 actual ID，保留 config-only 证据，不虚构解析。 |
| 未传 effective selector（包括 policy 选择 vendor default） | `auto` | 只由 `effective_selector=null` 导出；不查询或伪造 literal record，不比较默认选择，仍可记录 observed models。 |
...
只有完成 envelope 中稳定、结构化的模型字段可以进入 `observedModels`。Claude 的 canonical fixture 首选 `claude -p --output-format json` terminal completion envelope 顶层 `modelUsage` map 的 **own string keys**；这些 keys 才是 observed actual IDs，保持其原始枚举顺序后 first-seen 去重，绝不读取 entry 内的 `model` member。为兼容经 versioned adapter metadata/fixture 明确接受的旧/新 envelope，Claude parser 只可按以下固定顺序尝试同一“非 array object/map、至少一个 non-empty own string key”的 map contract：(1) `terminalEnvelope.modelUsage`（source `claude.result.modelUsage.keys`）；(2) `terminalEnvelope.result.modelUsage`（source `claude.result.result.modelUsage.keys`）；(3) `terminalEnvelope.usage.modelUsage`（source `claude.result.usage.modelUsage.keys`）；(4) `terminalEnvelope.usage.model_usage`（source `claude.result.usage.model_usage.keys`）。前一候选缺失、不是该 map、为空、含无效 key 或未被该 envelope version 的 metadata/fixture 接受时才继续下一候选；首个有效候选即停止，多个 path 不得合并。不得递归搜索任意 object，也不得把任何 scalar `model`、request echo 或 usage prose 当 fallback。四条 path 均不可用/无效时，Claude parser 不返回 `modelAttestation`。OpenCode 的唯一候选来源是其结构化 terminal result 中、adapter metadata 明确标为 actual identity 的 provider/model pair（以完整 `provider/model` 形式输出）；只出现 provider、只出现 bare model、request echo 或非稳定 event/text 时不组合、不猜测。两者都不得从 prompt、human-readable prose、billing 文案、PATH、日志文件名或模型目录反推名称。多个 vendor 报告的模型按原始首次出现顺序去重；这保留了一个任务可能使用多项模型的事实。无法读取、字段格式漂移、或只有非结构化文字时返回空列表和安全的 error code，而非抛出或编造 ID。
...
终态 handoff 的 `resolution_status` 必须恰为以下五种之一。`strict comparable` 表示 concrete record 的 vendor-discriminated expected identity 已通过 metadata validation，且 observed identity 能以对应 strict comparator 比较：Claude 为完整 opaque ID，OpenCode/structured 为完整 provider/model pair；它不是 bare-vs-namespaced、额外 namespace 或解析畸形情形。真值表如下，任何未列情况都是 `unverified`：

| selector kind / metadata | runtime observed evidence | 状态 | 必要条件 |
| --- | --- | --- |
| `concrete`，有效唯一 vendor-discriminated expected identity | 至少一项 strict comparable match | `exact` | Claude opaque ID 或 OpenCode/structured provider-model pair 的 `exact` 都只要求至少一个 match；即使另有非匹配项也不降为 mismatch。 |
| `concrete`，有效唯一 vendor-discriminated expected identity | 非空；每一项均有效、strict comparable，且全都不等于该 identity | `mismatch` | 这是产生 mismatch 的唯一条件。 |
...
| `alias` 带 expected identity、出现 `auto` record，或 `concrete` 的 declaration/kind/member/provider 不合该 vendor contract | 任意 | `unverified` | envelope 必须报 `metadata-envelope-malformed`、selector 归一为 `unknown`；不能以 runtime evidence 弥补无效 metadata。 |
...
当 policy 有意选择 vendor default 时，上表最后一行的 `requested_selector` 可以非空；它仍只是审计证据，不改变比较。格式损坏或严格 identity 歧义统一归入 `concrete-no-runtime-metadata`，除非未来受控 detail enum 专门细分；它绝不能被改写为 non-match。metadata 的 vendor/kind/member/provider 无效则更早归入 `metadata-envelope-malformed` 与 `selector-kind-unknown`，而非 runtime malformed。
...
`observed_models_json` 必须先以 JSON serializer 产生 JSON string，再由 **专用** YAML JSON-scalar encoder 以 double-quoted scalar 写入；该 encoder 不能调用或依赖现有 generic/plain `emitScalar`。它必须转义引号、反斜杠、换行、控制字符、`[`、`]`、`:`、`#` 及 document-marker-like 内容，禁止 string interpolation、plain scalar 或 flow sequence。reader 只接受解析结果为 string 的 scalar，再 JSON parse 为 `Array<string>`：必须是 array、不得为 `null`、且每个 member 都是 string；YAML flow-sequence、object、`null`、非 string member、截断/半写 scalar 或非法 JSON 一律丢弃并降级为空数组。fixture 必须 round-trip 覆盖 `[ ] : #`、单双引号、反斜杠、newline、Unicode、JSON `null` 与 non-array JSON。所有 frontmatter reader 必须容忍这些字段缺失、`observed_models_json` 不是合法 JSON、或数组中含非 string：读取时降级为空数组并显示 `unverified`，不得使 `--result`、`--progress` 或 dashboard 崩溃。`runResult` 必须把缺失/非法 `status` 规范为 display-only `unknown`，而不是调用 `undefined.toUpperCase()`；`runProgress` 必须仍能展示可读的 recent JSONL events，并标示 `frontmatter=unavailable`。没有 output file 仍是“未 dispatch”的正常错误，不属于该降级路径。新 writer 不得把原始 vendor result、完整 stderr 或 raw binary/config path 写入上述字段。
...
显式 recovery surface 是既有 `hopper-dispatch --probe <vendor>` 加新增、必须实现并测试的 `--recover-cache` flag：只有用户执行 `hopper-dispatch --probe <vendor> --recover-cache` 时，工具才可恢复。流程的唯一 commit point 是 atomic replace active cache；以下所有可失败准备必须在此之前完成：(1) inspect 原 active cache；(2) 在同目录以 owner-only 权限/ACL **创建时即**生成 fresh v1 temp，重新读取/assert 该权限后才完整 validate；若 temp ACL assertion 失败，best-effort 删除该 temp 后 abort；(3) 若 active 存在，以 owner-only 权限/ACL **创建时即**在同目录 exclusively 创建受控 backup，命名为 `<cache-basename>.recovery-<UTC-compact>-<8-lowercase-hex>.bak`。候选名已存在时绝不覆盖：生成新的 fresh 8-hex suffix，最多共尝试 8 个候选；第 8 个仍碰撞、或任一 exclusive creation 失败时 pre-commit abort 并返回 closed diagnostic `inventory-cache-recovery-backup-create-failed`。backup 创建后必须重新读取/assert owner-only 权限（Windows 使用当前用户 ACL 等价物）；该 ACL assertion 失败时 best-effort 删除这个新 backup 后 abort。若 active 缺失则不创建 backup，但仍必须完成 temp 的 owner-only assertion 与 validate；(4) 计算并执行 recovery backup retention/prune。retention 的 `<=3` 是成功 recovery 的 steady state：按 parsed UTC timestamp 由旧到新排序，相同 timestamp 以完整 backup basename 的 ordinal bytewise order 作稳定 tie-breaker，删除最旧的**已有** backup 以使总数不超过 3；本次 recovery 新创建的 backup 永远不是 prune candidate。prune 必须在 replace 前完成。prune 失败则 pre-commit abort，active 仍 byte-for-byte unchanged，且 backup 总数可暂时超过 3；下一次显式 recovery 必须重新计算 eligible 旧 backup 并自行修复该暂时超额，仍不得 prune 该次新建 backup。若 prune 已成功而随后在 replace 前 crash/fail，active 仍 byte-for-byte unchanged，已删 backup 不被“rollback”恢复，temp 仍只 best-effort cleanup，已创建且未被 prune 的 backup 可以保留。任一其余 pre-commit step 失败时 active cache 同样必须 byte-for-byte unchanged，temp cleanup 仅 best-effort，已经创建的 backup 可以保留。只有准备全部成功后才 atomic replace；若该 atomic replace 在 commit point 前报告失败，返回 closed diagnostic `inventory-cache-recovery-replace-failed`，active 必须 byte-for-byte unchanged，并适用相同 backup/temp cleanup 规则。replace 后不得进行 retention/prune 或声称能 rollback。

atomic replace 已成功、但其后的 fsync/目录 durability 确认失败时，才返回独立 closed diagnostic `inventory-cache-recovery-durability-unknown`：active cache **可能已经是新内容**，不得声称 rollback、旧内容仍在或 recovery 未提交。它严格不同于 `inventory-cache-recovery-replace-failed`：后者尚未跨越 commit point，active 已知 byte-for-byte unchanged；前者已经跨越 commit point，内容只是不再能确认 durability。crash-before-commit（包括 prune 成功但 replace 尚未成功）时 startup 只按旧 active（或 missing）状态 reconcile，best-effort 清理 temp 并允许已有 backup 保留；crash-after-commit 时 startup 只验证 active 的 fresh v1 内容并保留 backup，绝不自动回滚。普通 dispatch/probe 永不隐式启动此流程。若现有 probe 层不能承载 flag，本阶段实现范围必须增加同语义的 `hopper-dispatch --recover-cache <vendor>` command 和等价测试，不能仅保留文字约定。
...
| dashboard vendor inventory/card | 与 CLI 同一 safe provenance、binary availability/basename、`sourceKind`/`sourceLabel`、safe diagnostic code，以及每个 direct `vendors[]` record 上固定的四个 inert legacy safe-shim keys | `binaryPath`、`notes`、`cacheError`、`modelsSource` 的任何 raw value，以及 config/PATH/stderr/auth/provider value |
...
Resolution: alias-resolved (source: claude.result.modelUsage.keys)
...
- public cache projection 是唯一 renderer 输入：CLI 的 `--models`/`--setup`/`--capabilities`/`--check-model` 与 dashboard API/card 都使用它，不得直接读取 `binary_path`、`models_source`、`notes` 或 raw cache error。inventory contract v2 永久删除/禁止的是 `binaryPath`、`notes`、`cacheError` 与 `modelsSource` 的**raw legacy values**，并以 `binaryAvailability`、`binaryBasename`、`sourceKind`、`sourceLabel`、`diagnosticCode` 和 `diagnosticState` 提供安全替代；当前 v2 不删除四个 legacy key names，因为每个 direct `vendors[]` vendor record 必须保留其 exact inert safe shim。任何 response depth 均不得为了兼容旧 card/client 返回 raw legacy value。
...
- 当前 v2 阶段的 dashboard rollout 必须先发布 null-safe client；feature gate 只控制 card/v2 rendering 的启用，**不控制**永久 shape-compatible safe shim。每个 direct `vendors[]` vendor record 在 gate on、gate off、未启用 card 与 rollback bundle 中都必须保留四个 direct legacy keys：`notes: []`、`cacheError: null`、`modelsSource: null`、`binaryPath: null`。这四个 key names 不是本阶段要删除的 legacy values；raw legacy values 已被永久禁止/删除。递归 response scan 必须断言每个 direct vendor record 都有这四个 exact safe values，且只允许它们在该明确 vendor-record 路径出现；同名 key 出现在 root、任何 nested object、array item、diagnostics/cache/metadata 子对象或任何其他路径时一律失败，任何其他 value 也是隐私失败。old SPA 对这些值及 missing/null/unknown v2 enum 只能显示 unavailable，不能崩卡；new client 不得把从 old server 收到的 raw legacy input 复制到 card、API output 或任何其他 public surface。只有未来的 breaking contract 才可删除这四个 key names，超出本阶段范围，不以不可验证的 coverage gate 作为当前 rollout 条件。`inventoryContractVersion` 只作为 compatibility diagnostic，缺失、未知或未来值不得成为 hard assertion/throw。外部 consumer 的 deprecation notice 必须说明安全替代字段。rollback 只能保持 safe shim、关闭 card rendering 或回滚到仍不暴露 raw field 的 client/server bundle；不得以恢复任何 legacy raw field 作为回滚手段。
- `inventoryContractVersion` 的完整 v2 fixture 必须包含版本字段、六个 v2 public fields、至少一个完整 direct `vendors[]` record 及其四个 safe-shim keys，并配套版本缺失、未知和 future 的 fixture，后三者只能降级为 compatibility diagnostic，不能 throw、删 key 或放回 raw value。
...
实施 landing sequence 是强制顺序，不是扩大本协议的额外 lifecycle feature：先冻结 vendor-discriminated types/metadata 与 strict comparator contract，再让 parser-to-finalizer 数据流收口，然后写 file-backed protocol/readers；其后先完成 cache recovery transaction（含创建即 owner-only 的 temp/backup、ACL-delete-on-assert-failure、8-candidate exclusive backup collision、pre-commit replace-failure、post-commit durability、prune-before-crash 与 retention self-heal fixtures），再接 cache public projection 与 dashboard v2 safe shim，并在每步完成对应 fixture/compat 断言后进入下一步。共享 lifecycle 文件的阶段边界、revalidation、idle/terminal sequencing 仅作为实施检查项：必须确认新字段没有改变一次 spawn、唯一 terminal event、resume/revalidation 或 timeout 行为，但本设计不新增 lifecycle phase、状态转换或跨任务协议。

### Phase 1：合同与数据路径

1. **Types、adapter metadata、比较边界：** 在 `cli/src/types.js` 的 adapter/result contract 加 raw/effective selector provenance、`alias|concrete|auto|unknown` selector metadata 与 optional `modelAttestation`；实现 versioned selector-metadata envelope/schema、binding validator 和每个**非 null** selector record 的 exact-literal vendor-discriminated union（`auto` 只由 `effective_selector=null` 导出，不是 record；alias 无 expected identity；Claude concrete 仅 `{identity_kind:"opaque-id",id}`；OpenCode/已声明 structured adapter concrete 仅 `{identity_kind:"provider-model",provider,model}`）。每个 adapter declaration 都含 `identity_kind`，structured adapter 还须有互斥的 live provider registry 与 sentinel closed set；禁止 sentinel/dead/unregistered provider 与 cross-vendor kind。无效 identity 必须成为 `metadata-envelope-malformed`/`unverified`。Claude/OpenCode 只从 sanitized/versioned capabilities cache 或 adapter 发布的 versioned manifest 零 spawn 分类，Kimi 仅声明可证实的 exact record。扩展 `cli/src/model-normalize.js` 为唯一 comparison boundary，令 `cli/src/model-check.js` 与 `cli/src/dispatch.js` 调用明确分离的 validation/strict-runtime modes；strict mode 不得调用 `modelKeysMatch`，adapter 不实现第二个 canonicalizer。
2. **所有 parseResult 消费点：** Claude parser 首选 terminal completion envelope 顶层 `modelUsage` map own keys，并只按本设计固定的 metadata/fixture-accepted fallback path 顺序读取；不得读取 entry `model` member、scalar request echo 或递归自由文本。OpenCode parser 仅接受其结构化 terminal result 中经 metadata/fixture 验证的完整 provider/model pair；Kimi 只保留可证实的 config-only evidence，直至其 result 有稳定 actual-model 字段。`cli/bin/hopper-runner` 和 `cli/src/dispatch.js` 都必须透传 optional `modelAttestation`，而不是只取 text/status/usage；任何缺失、request echo 或 schema drift 都产生无 runtime evidence。
...
5. **Cache：** `cli/src/cache.js` 保持 v1，先加入 diagnostics-aware reader 以区分 missing、ok-v1、version mismatch 与 malformed，再加入 owned-field additive merge、unknown-field preservation 和 version-mismatch-as-unknown/no-write/byte-preservation 行为；实现并测试 `hopper-dispatch --probe <vendor> --recover-cache`（或等价的新 `--recover-cache <vendor>` command）的创建即 owner-only temp/backup、ACL assertion 失败删除、exclusive backup 与 8-candidate collision 的 `inventory-cache-recovery-backup-create-failed`、timestamp+basename tie-broken retention（不 prune 当前 recovery backup）、failed-prune 后下一次 recovery self-heal、prune-before-crash、`inventory-cache-recovery-replace-failed` 的 pre-commit unchanged active 与严格独立的 post-commit durability unknown。普通 setup/capabilities/probe 仅返回 closed diagnostic+hint，绝不写。完整 `inventoryContractVersion` fixture（版本字段、六个 v2 fields、direct vendor safe shim，及 missing/unknown/future variants）必须与这组 cache/public-projection fixtures 一起通过。`cli/src/vendor-probe/kimi.js` 及其他 probe 只产生 canonical closed source label/diagnostic code；它们不将 config path、stderr、auth 或 provider text 放入 public entry。
6. **Dashboard privacy migration：** 先发布 null-safe client；feature gate on/off、card-disabled 与 rollback 均保持每个 direct `vendors[]` record 的永久 shape-compatible safe shim（`notes=[]`、`cacheError=null`、`modelsSource=null`、`binaryPath=null`），且 recursive scan 永不允许 raw legacy value。只有未来 breaking contract 可删除这些 key names，不属于本阶段覆盖门禁。随后验证 recursive closed-set response projection、完整 `inventoryContractVersion`、以及 old/new server-client 的 missing/null/unknown/future-enum rendering。更新 `dashboard/server/routes/vendors.js`、API schema、`dashboard/client/src/lib/types.ts` 与 `VendorCard` 只传输/读取六个 v2 safe binary/source/diagnostic closed fields；发布说明、compat behavior 与不重新泄露 raw field 的 rollback procedure 是此步骤的交付物。
...
| 类别 | 场景 | 必要断言 |
| --- | --- | --- |
| Claude alias | request `fable`，runtime 顶层 `modelUsage` map keys 枚举 `claude-*` | `selector_kind=alias`、observed array 保留、`alias-resolved`，不产生 mismatch。 |
...
| Claude absent metadata | alias 成功但四条允许的 `modelUsage` map path 均无有效 map | `unverified`，不由 scalar usage、entry `model`、prose 或 knownGood 推断。 |
| Claude/OpenCode zero-spawn selector metadata | schema v1 cache envelope/manifest、vendor-adapter-catalog binding、validity/time/version、每条完整非 null selector literal、alias/concrete discriminated record；`effective_selector=null` 不查 record 而导出 auto；Claude concrete 仅 `{identity_kind:"opaque-id",id}`，OpenCode concrete 仅 `{identity_kind:"provider-model",provider,model}`；duplicate/conflicting literal、unexpected auto record、unknown forward field；fixtures 覆盖 `fable`、`sonnet`、`sonnet[1m]`、`best`、`default`、`opusplan`、未枚举 `[N_unit]` | dispatch 不 spawn/probe/network；只对 post-policy non-null effective selector 用有效 envelope 分类，且不建立 alias→actual 映射；unsupported schema/adapter mismatch/过期/缺失/malformed/duplicate/未枚举为 `selector_kind=unknown`、`unverified`，绝不 false `mismatch`/`alias-resolved`。 |
| Expected identity union validation | alias 携带 expected identity、出现 auto record；Claude 使用 provider-model；OpenCode/structured 使用 opaque-id；concrete 缺/多 member、unknown/declaration-mismatched kind、sentinel/dead/unregistered provider 或 non-live structured provider | 每项均为 `metadata-envelope-malformed`、`selector_kind=unknown` 与 `unverified`，不调用 strict comparator，不能由 observed value 修复。 |
| OpenCode concrete | request `provider/model`，runtime 同/不同 ID，且 record 为唯一 provider-model identity | 分别为 `exact`/`mismatch`；比较仅用 `model-normalize` 的 strict runtime path。 |
| Strict identity forms | Claude opaque ID exact/non-match；OpenCode/structured 同 provider/model、不同 provider、bare model、额外 namespace、单边 qualified | 两种 vendor-discriminated kind 都覆盖；仅完整 Claude opaque ID 或相同 provider+model pair match；bare 与 namespaced 永不等价，歧义/不可比较为 `unverified`；不得调用 `modelKeysMatch`。 |
| Concrete status truth table | 两种 identity kind 各自的 one strict match 加其它不匹配项、全部 comparable non-match、空 observed、任一 malformed/ambiguous/bare-vs-namespaced 不可比较项、metadata expected identity 缺/坏 | 对每个有效 kind 有至少一个 match 即 `exact`；只有所有 observed 都有效可比较且全部非等才 `mismatch`；identity kind 无效为 metadata malformed，其余不可比较为 `unverified`。 |
...
| Auto | 未传 `--model` 或 policy 清空 effective selector，且有或无 observed model | `effective_selector=null` 时直接导出 kind `auto`，不读取/匹配 literal record；status `unverified`，observed value 仍可见。 |
| Multi-model | runtime 返回重复或多个 IDs | stable first-seen de-dup，JSONL array 与 frontmatter JSON string round-trip 一致。 |
| Parser contract | Claude 顶层 `modelUsage` map own keys、四条固定 path 的 version-compatible fallback、前一有效 path 胜出、absent/empty/array/invalid-key/request-echo/entry-`model` fixture；OpenCode structured provider/model pair | `modelAttestation` 只来自允许的 structured map key/pair；Claude 不递归搜索、不合并 path、也不读取 entry/scalar `model`；sync 与 background consumer 不丢失；缺失/歧义时不 throw，且不伪造 exact/alias-resolved/mismatch。 |
...
| Cache/probe semantics | missing cache、旧 v1 entry、含 root/vendor/nested unknown future field 的 v1 entry、version mismatch、malformed cache、ordinary setup/capabilities/probe、`--probe <vendor> --recover-cache` success/failure、创建即 owner-only temp/backup、ACL assertion 删除、exclusive backup name/8-candidate collision/retention/backup failure、replace failure、完整 `inventoryContractVersion` fixture（version、六个 v2 fields、direct vendor shim）及 missing/unknown/future version variants | diagnostics-aware reader 区分 missing/ok-v1/mismatch/malformed；ordinary surface 只给 closed diagnostic+recover hint且不写；仅 missing 初始化、v1 owned fields additive merge 且所有 unknown field 保留；mismatch/malformed 为 unknown、普通 writer no-write，version mismatch 原文件字节不变；explicit recovery 仅在 owner-only temp/backup assertion 成功后 atomic replace fresh-v1，8 个 candidate 全碰撞/backup exclusive creation failure 使用 `inventory-cache-recovery-backup-create-failed`，固定 retention 以 timestamp+basename stable order 且不 prune 当前 recovery backup，任何 backup failure abort且不降格为 missing；replace failure 使用 `inventory-cache-recovery-replace-failed`；完整 `inventoryContractVersion` response 仍只使用 safe projection/shim，version variants 不 throw 或泄露 raw value。 |
| Source pairing | 每个允许的 `vendor × sourceKind × sourceLabel` 组合、未声明 vendor/source 组合、future source kind | 仅 allow-map 中的 pair 可公开；错配/future 一律 `sourceKind=unknown`、`sourceLabel=unknown`，所有 public surface 一致。 |
| Recovery transaction (F2) | existing-cache inspect、创建即 owner-only temp validate、exclusive owner-only backup/ACL assertion、ACL-delete-on-failure、8-candidate collision、retention-prune 任一步失败；equal-timestamp backup names；当前 recovery backup 不可 prune；prune 后、replace 前 crash；atomic replace failure；missing cache；crash after replace；post-commit fsync/durability failure | temp/backup ACL success 必须已断言，失败对象必须删除；第 8 个 fresh candidate 仍碰撞或 exclusive create 失败使用 `inventory-cache-recovery-backup-create-failed`；retention 按 timestamp 再 basename tie-break，成功 recovery 的 steady state 为 `<=3`，但 prune failure 可暂时超过且下一次 recovery self-heal；prune-before-crash 后 active 仍 byte-for-byte unchanged且不恢复已删 backup；所有 pre-commit failure 与 replace failure 都保持 active byte-for-byte unchanged、temp best-effort cleanup、已有 backup 可保留，replace failure 使用 `inventory-cache-recovery-replace-failed`；missing cache 不建 backup仍 temp validate+atomic install；成功 replace 是唯一 commit point，after-crash 只验证新 active、绝不自动 rollback；仅成功 replace 后的 durability failure 使用 `inventory-cache-recovery-durability-unknown` 且承认 active 可能已更新。 |
...
| Rendering/privacy/dashboard migration | `--models`、`--setup`、`--capabilities`、`--check-model`、result/progress、完整 `inventoryContractVersion` permanent safe-shim/v2 fixture（version、六字段、direct vendor shim）及 missing/unknown/future version variants、old/new server-client matrix、six-field future-enum、feature-gate-off 与 rollback fixture | 六个 v2 public field 各自断言 closed/unknown(nullable basename)；每个 public surface 的 `source_label`/`diagnosticCode` 都断言为 canonical closed enum，unknown/future/raw 值归一 unknown；API response 的 recursive closed-set key/value scan 在每一嵌套 depth 断言 forbidden `sourceNote`、raw path/URL/stderr/auth/provider values，并断言**每个** direct `vendors[]` record 都有 `notes=[]`、`cacheError=null`、`modelsSource=null`、`binaryPath=null` 四个 exact safe value；同名 nested key 一律失败。gate off、card disabled 与 rollback 同样不得返回 raw legacy value；old client 不崩卡，新 client 不复制 old-server raw legacy input，`inventoryContractVersion` 仅 diagnostic（missing/unknown/future 不 throw）；只有未来 breaking contract 可删除 shim key names。 |
