...
| `selector_kind` | adapter 声明的 `alias`、`concrete` 或 `auto`。`auto` 表示没有显式 selector。 | adapter 的 declarative selector metadata，而非正则猜测 |
...
原始 selector 永远保留用于审计和显示。adapter 只声明合法 alias 集、其余值的默认 `concrete` 分类，以及 runtime result 中可读取的 metadata path；adapter 不实现或调用独立的 ID 比较器。
...
2. **runtime attestation：** 使用同模块导出的严格 comparison path。它只做 trim 与该 vendor 已文档化的大小写/namespace 等价；不得 strip provider prefix、tail-match、fuzzy-match 或把 alias 展开为 ID。只有这个 path 能决定 concrete 的 `exact` 或 `mismatch`。
...
--model / policy selector
        │
        ▼
model-normalize.js (selector kind + one comparison boundary)
...
`requested_selector` 从 argv/policy resolution 进入启动 snapshot；不得由 observed value 回填。catalog provenance 从已存在的 cache 只读进入 snapshot；不得触发 probe。runtime attestation 只从同一次 parsed vendor result 进入 terminal writer。读侧先确认 frontmatter terminal completeness，再把 JSONL 作为 supplementary evidence，避免两个文件的不同写入时刻被混同为一个原子事实。
...
adapter 必须在其 model capability metadata 中为可传入 `--model` 的选择器声明 alias 集和默认分类，而不是在 dispatcher 中根据字符串格式推测。该 metadata 只分类，实际 comparison 仍完全由 `model-normalize.js` 执行。

| Vendor 场景 | `selector_kind` | 比较规则 |
| --- | --- | --- |
| Claude `fable`、`sonnet`、`opus`、`haiku` | `alias` | 可运行且 observed actual model 非空即 `alias-resolved`；不要求字面相等。 |
| Claude 明确完整 ID，例如 `claude-sonnet-4-6` | `concrete` | 与 observed actual IDs 做 `model-normalize` strict runtime comparison。 |
| OpenCode `provider/model` | `concrete` | provider/model 与 observed actual IDs 做 `model-normalize` strict runtime comparison。 |
...
adapter 可以把未来 vendor 特有的合法 alias 声明为 `alias`，但不得把任意未知自由文本乐观地标为 alias。未知 selector 的分类应随 vendor contract 保守地为 `concrete`；CLI validation 的既有行为不变。每个 adapter 的 metadata、其 parser 和 types contract 都要有 fixture test，防止新增 adapter 忘记声明 selector kind。
...
`ParsedVendorResult`/等价 adapter result type 必须把 `modelAttestation` 定义为 optional；所有 `parseResult` consumers 都不得丢弃它：后台 `hopper-runner` 将它传给 terminal event/frontmatter resolver，`cli/src/dispatch.js` 的同步 `runVendorTask` 原样返回它给 output writer/调用者。失败 result 可省略该字段；消费者必须把省略规范化为无 runtime evidence，而不是因为可选字段不存在而 throw。
...
| 优先级 | 条件 | 状态 | 含义 |
| ---: | --- | --- | --- |
| 1 | `selector_kind=concrete`，且有权威 observed models，但严格 runtime comparison 没有命中 requested selector | `mismatch` | 证据明确与用户的 concrete 请求不符。 |
| 2 | `selector_kind=concrete`，且至少一个 observed model 与 requested selector strict runtime match | `exact` | concrete 请求被运行时元数据精确证明。 |
...
所有 frontmatter reader 必须容忍这些字段缺失、`observed_models_json` 不是合法 JSON、或数组中含非 string：读取时降级为空数组并显示 `unverified`，不得使 `--result`、`--progress` 或 dashboard 崩溃。`runResult` 必须把缺失/非法 `status` 规范为 display-only `unknown`，而不是调用 `undefined.toUpperCase()`；`runProgress` 必须仍能展示可读的 recent JSONL events，并标示 `frontmatter=unavailable`。没有 output file 仍是“未 dispatch”的正常错误，不属于该降级路径。新 writer 不得把原始 vendor result、完整 stderr 或 raw binary/config path 写入上述字段。

### progress JSONL

不新增第二个 terminal event，也不以 attestation 刷新 idle activity。既有唯一 terminal event 可包含以下 optional fields，JSONL 中 `observed_models` 保持真正 array。`cli/src/progress.js` 的 `OPTIONAL_EVENT_FIELDS` 必须显式 allowlist `requested_selector`、`selector_kind`、`observed_models`、`model_attestation_source`、`model_attestation_observed_at`、`resolution_status` 与 `resolution_detail`；没有这一项，writer 会静默丢弃这些 event fields：
...
frontmatter 是唯一的 terminal handoff canonical record，JSONL 是追加式 progress evidence。reader 的先后与中途写入规则如下：
...
2. JSONL 已有 terminal event、但 frontmatter 仍是 `in-progress`、缺 status、损坏或尚未写入时，视为 `handoff_state=finalizing|partial`。可显示事件中的 observed model 作为 diagnostic，但 resolution 必须为 display-only `unverified`，不能把该 event 单独提升为最终 `exact`、`alias-resolved` 或 `mismatch`。
...
这一定义避免 terminal JSONL 与 frontmatter 原子写的间隙造成错误终态，同时保留一次 terminal event 合同。

### Probe cache

cache 保持 schema v1；provenance 是 vendor entry 中的 optional additive field，因此本阶段不得 bump `CACHE_VERSION`。新 reader 以 non-mutating normalizer 读取旧 entry：缺 provenance 的 entry 渲染为 `source=unknown`、`freshness=unknown`、`binary_availability=unknown`，而保留其既有 models、timestamp 和其他可读字段。`setVendorCache` 必须 merge 同 vendor 的 unknown/forward-compatible fields，不能因为新 probe result 没有某个新增字段而丢弃它。

对未来真正的 cache version mismatch，dispatch、result 与 setup 不采用任何 cached model evidence，统一显示 `cache_schema=unknown`/`catalog=unknown`；它们不得自动删除、重建或覆盖旧文件。`--models` 可以返回受控的 `cache-version-unsupported` 诊断，但也不能泄露 cache absolute path。只有一个专门的、显式设计过的迁移/重建流程才可替换不兼容 cache。
...
| `--result <task>` | requested selector、kind、observed actual models、resolution status、attestation source/time、启动时 catalog/binary snapshot；partial handoff 的安全状态 | 原始 result envelope、完整 raw log、binary/config 路径；除非既有 `--full` 合同明确请求原始 vendor output |
| `--progress <task>` | in-progress 时 requested selector/kind 与启动 snapshot；完成后加 terminal attestation；partial/finalizing 状态 | 将 heartbeat 当作 attestation、binary/config 路径、未完成时的 final resolution conclusion |
...
- public cache projection 是唯一 renderer 输入：CLI 的 `--models`/`--setup`/`--capabilities`/`--check-model` 与 dashboard API/card 都使用它，不得直接读取 `binary_path`、`models_source` 或 `notes`。dashboard API 删除 `binaryPath` 与 raw `notes` 字段，改为 `binaryAvailability`、`binaryBasename`、`sourceKind`、`sourceLabel` 和可枚举 `diagnosticCode`。
...
1. **Types、adapter metadata、比较边界：** 在 `cli/src/types.js` 的 adapter/result contract 加 `selector_kind` metadata 与 optional `modelAttestation`；Claude/OpenCode/Kimi adapter 声明 alias/concrete 分类和 result metadata path。扩展 `cli/src/model-normalize.js` 为唯一 comparison boundary，令 `cli/src/model-check.js` 与 `cli/src/dispatch.js` 调用其 validation/runtime 两种明确模式；adapter 不实现第二个 canonicalizer。
2. **所有 parseResult 消费点：** Claude parser 优先读取稳定 `modelUsage`/等价结构化运行时字段；OpenCode parser 仅在其 result 有稳定 concrete metadata 时记录；Kimi 只保留可证实的 config-only evidence，直至其 result 有稳定 actual-model 字段。`cli/bin/hopper-runner` 和 `cli/src/dispatch.js` 都必须保留 optional `modelAttestation`，而不是只取 text/status/usage。
3. **File protocol：** `cli/src/background.js` 负责启动 snapshot 与 scalar-safe `observed_models_json`；`cli/bin/hopper-runner` 负责把 parsed attestation 写入唯一 terminal JSONL event 和 terminal frontmatter；`cli/src/progress.js` 更新 `OPTIONAL_EVENT_FIELDS`。实现遵从本设计的 frontmatter-canonical/finalizing precedence，且不改变 idle/ceiling 或 terminal-event 数量。
4. **Read surfaces：** `cli/bin/hopper-dispatch` 的 result/progress reader 对缺/坏 frontmatter/status 使用安全降级；models/setup/capabilities/check-model 全部使用 public cache projection。`dashboard/server/routes/vendors.js`、`dashboard/client/src/lib/types.ts` 与 `VendorCard` 改为安全 binary/source/diagnostic 字段，不能再传输 `binaryPath`/raw `notes`。
5. **Cache：** `cli/src/cache.js` 保持 v1，加入 read-compatible provenance normalizer、per-vendor merge-preservation 和 version-mismatch-as-unknown 行为。`cli/src/vendor-probe/kimi.js` 及其他 probe 只产生 allowlisted source label/diagnostic code；它们不将 config path、stderr、auth 或 provider text 放入 public entry。
6. **镜像与验证边界：** 根 `cli/` 与 `plugins/hopper/cli/` 是发布镜像，实施时两套受影响代码必须保持字节级同步并由现有镜像 hash test 验证；本设计不授权只改其一。
...
| Legacy/missing handoff | no frontmatter、缺/非法 `status`、坏 `observed_models_json` | result/progress 不 throw、不调用 undefined method；显示 `unknown`/`unverified` 或 `frontmatter=unavailable`，仍可展示有效 events。 |
| Mid-write protocol | terminal JSONL 已写但 frontmatter in-progress、frontmatter terminal但JSONL缺失/partial、两者冲突 | 分别验证 `finalizing` display-only unverified、frontmatter canonical、`attestation_consistency=conflict`；JSONL partial line 被忽略。 |
| Optional-event allowlist | 终态 event 带所有新 attestation fields | `OPTIONAL_EVENT_FIELDS` 保留每一字段，JSONL round-trip 不丢值。 |
| Cache/probe semantics | cache stale、旧 v1 entry、version mismatch、probe error、config source、CLI catalog | 无自动 spawn；v1 optional provenance 不 bump/不丢字段；mismatch 为 unknown 且不覆盖 cache。 |
...
| Rendering/privacy | `--models`、`--setup`、`--capabilities`、`--check-model`、result/progress 与 dashboard fixture | 仅显示 allowed safe fields；API 不含 `binaryPath`/raw `notes`；断言不存在 absolute path、home path、PATH、stderr、auth/provider 或 secret-like content。 |
| Parser contract | Claude/OpenCode/Kimi metadata present/absent/malformed；sync 与 background consumer | optional `modelAttestation` 不丢失；缺失时不 throw；只有结构化 runtime field 可进入 observed models。 |
