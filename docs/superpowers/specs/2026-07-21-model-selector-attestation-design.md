...
对未来真正的 cache version mismatch，normalizer、dispatch、result 与 setup 不采用任何 cached model evidence，统一降级显示 `cache_schema=unknown`/`catalog=unknown`；普通 `setVendorCache` 也不得向该文件写入、自动删除、重建或覆盖旧文件，且必须保留原 cache 文件的字节内容不变。`--models` 可以返回受控的 `cache-version-unsupported` 诊断，但也不能泄露 cache absolute path。只有一个专门的、显式设计过的迁移/重建流程才可替换不兼容 cache。
...
- v2 的安全映射是固定且封闭的：`modelsSource` 只能投影为 allowlisted `sourceKind`（`cli-catalog|config|adapter-aliases|static|unavailable|error|unknown`）和 allowlisted `sourceLabel`；`binaryPath` 只能投影为 `binaryAvailability`（`present|missing|unknown`）及已验证 `binaryBasename`; `notes` 与 `cacheError` 只能投影为 `diagnosticCode`（`none|probe-failed|catalog-unavailable|cache-version-unsupported|cache-malformed|runtime-model-metadata-absent`）和 `diagnosticState`（`none|degraded|unavailable|unknown`）。任何未映射或未来 raw field 都必须被丢弃，不得改名后藏在 `vendor`、`cache`、`diagnostics`、`metadata`、model entry 或其他嵌套响应对象中。原始路径、stderr、provider/auth/config/binary source、异常文字和账户 identity 均不得通过任何 response depth 回流。
- dashboard server、API schema/types 和 `VendorCard` 必须 lockstep rebuild/release：先在同一变更中令 server 只投影 v2 safe fields、client 只读取这些字段，并以 `inventoryContractVersion: 2` 显式协商/断言。新 client 面对 old server 的 legacy `notes`/`cacheError`/`modelsSource`/`binaryPath` 必须忽略它们；old client 面对 v2 missing legacy fields、`null`、`undefined` 或 safe-enum unknown 值必须 null-safe 地显示 unavailable，不能抛错或要求 server 回吐 raw value。外部 consumer 的 deprecation notice 必须说明 v1 raw fields 已删除及安全替代字段。rollback 只能回滚整套 server/client 到相互兼容的、仍不重新暴露 raw fields 的 build，或保持 v2 safe projection 并关闭新 card rendering；不得以恢复任何 legacy raw field 作为回滚手段。
...
| Rendering/privacy/dashboard migration | `--models`、`--setup`、`--capabilities`、`--check-model`、result/progress、dashboard v2 fixture、old/new server-client matrix 与 rollback fixture | 仅显示 allowed safe fields；API response 的 recursive key/value scan 在每一嵌套 depth 断言不存在 `binaryPath`、`notes`、`cacheError`、`modelsSource`、absolute path、home path、PATH、stderr、auth/provider 或 secret-like content；仅允许安全 enums。old client 对 missing/null/unknown legacy field 显示 unavailable，新 client 忽略 old-server raw legacy field，均不得 throw；rollback bundle 仍不重现 raw fields。 |
