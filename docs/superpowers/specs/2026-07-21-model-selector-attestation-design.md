...
内部 `source_kind` 与 public `source_label` 的映射也是闭集：`cli-catalog` 仅在 OpenCode verified catalog 时映射 `opencode-cli-catalog`，`config` 仅在 Kimi configured-alias source 时映射 `kimi-configured-aliases`，`adapter-aliases` 映射 `claude-selector-metadata`，`static` 映射 `adapter-static-selectors`，`unavailable` 映射 `unavailable`，`error|unknown` 或 vendor/source 组合不匹配映射 `unknown`。任何 future internal value 不得直接公开，只能映射 `unknown`。

canonical storage field 为 `diagnostic_code`，public renderer 输出同值为 `diagnosticCode`。它的闭集为 `none|runtime-model-metadata-conflict|runtime-model-metadata-malformed|runtime-model-metadata-absent|selector-metadata-cache-adapter-mismatch|selector-metadata-cache-schema-unsupported|selector-metadata-cache-expired|selector-metadata-cache-missing|inventory-cache-version-unsupported|inventory-cache-malformed|capability-failed|probe-failed|catalog-unavailable|unknown`。选择单个 code 时按下表由上至下取第一个匹配项；后续 raw errors 不改变选择结果：

| 优先级 | 触发条件 | `diagnostic_code` |
| ---: | --- | --- |
| 1 | 同一 task 的结构化 runtime model identity 相互冲突 | `runtime-model-metadata-conflict` |
| 2 | runtime model field/pair 类型或 schema 畸形 | `runtime-model-metadata-malformed` |
| 3 | selector metadata 的 vendor、adapter id 或 adapter version 不匹配 | `selector-metadata-cache-adapter-mismatch` |
| 4 | selector metadata schema version 不在支持集 | `selector-metadata-cache-schema-unsupported` |
| 5 | selector metadata 已过期、时间戳无效或 validity 不为 accepted | `selector-metadata-cache-expired` |
| 6 | 无可用 selector metadata source | `selector-metadata-cache-missing` |
| 7 | inventory cache version 不支持 | `inventory-cache-version-unsupported` |
| 8 | inventory cache 解析/结构畸形 | `inventory-cache-malformed` |
| 9 | 已知 capabilities command 失败 | `capability-failed` |
| 10 | 已知 probe spawn/exit/parse failure | `probe-failed` |
| 11 | 无安全 catalog | `catalog-unavailable` |
| 12 | runtime result 成功但没有稳定 actual-model metadata | `runtime-model-metadata-absent` |
| 13 | 其他无法可靠分类的错误 | `unknown` |
...
分类输入必须是下列 schema v1 的已 sanitize metadata envelope；schema fields 缺失、类型不符或值不在闭集即拒绝。`schema_version` 当前只支持 `1`，future version 不做 best-effort parse：归为 `selector-metadata-cache-schema-unsupported` 并得到 `unknown`。结构示意如下：
...
```json
{
  ...
  "selectors": [
    {
      "literal": "exact selector literal",
      "kind": "alias | concrete",
      "variants": [
        { "type": "suffix-literal", "suffix": "[1m]" },
        {
          "type": "identity-grammar",
          "grammar": {
            "type": "opaque-id | provider-model",
            "separator": "/",
            "allowed_providers": ["explicit literal provider"],
            "allowed_models": ["explicit literal model"]
          }
        }
      ]
    }
...
`identity-grammar` 是 discriminated union：`opaque-id` 只接受完整 literal ID；`provider-model` 才能有 `/` separator，且 `allowed_providers` 和 `allowed_models` 必须都是 metadata 显式列出的 closed literals。`suffix-literal` 只接受 exact suffix literal；例如 `[1m]` 只在数组中明确列出时有效。若未来确需 `[N_unit]` 形式，metadata 必须使用一个新的、受支持的 discriminant，并显式给出 closed `allowed_numbers` 与 `allowed_units`；不得使用任意 regex、开放数字、开放 unit 或字符串形状匹配。未声明的 `[N_unit]` 一律不匹配。该 schema 永不包含 alias→actual model 映射。

zero-spawn source priority 是：(1) vendor、adapter id、adapter version、catalog version 都匹配且未过期的 sanitized capabilities-cache envelope；(2) 匹配同一 binding 的 adapter-manifest envelope；(3) 无有效 source。dispatch 只读该两项已有本地数据，绝不为了分类执行 CLI、probe 或网络请求。每个 envelope 必须 `generated_at <= expires_at` 且 `validity=accepted`；vendor/adapter mismatch、schema unsupported、过期/无效时间、缺失或 malformed envelope 均拒绝。cache 对未知 future fields 的读取策略是：已支持 schema 内忽略但不解释 unknown fields，兼容 writer 仅在同 schema additive merge 时保留它们；schema version 不支持时不读、不分类、不覆盖。只有在两项 source 都无效或 selector/variant 未枚举时，effective selector 为 `unknown`/`unverified`，不得产生 `mismatch` 或 `alias-resolved`。

| Vendor 场景 | `selector_kind` | 比较规则 |
| --- | --- | --- |
| Claude metadata 枚举的 profile/alias（可包括 `fable`、`sonnet`、`best`、`default`、`opusplan`、`[1m]` 及未来枚举值） | `alias` | 仅在有效 metadata 明确声明这些 literal/variant 形式时分类为 alias；可运行且 observed actual model 非空即 `alias-resolved`，不要求字面相等，也不允许硬编码到真实模型 ID。 |
...
对未来真正的 cache version mismatch，normalizer、dispatch、result 与 setup 不采用任何 cached model evidence，统一降级显示 `cache_schema=unknown`/`catalog=unknown`；普通 `setVendorCache` 也不得向该文件写入、自动删除、重建或覆盖旧文件，且必须保留原 cache 文件的字节内容不变。`--models` 可以返回受控的 `inventory-cache-version-unsupported` 诊断，但也不能泄露 cache absolute path。显式 recovery surface 是既有 `hopper-dispatch --probe <vendor>` 加新增、必须实现并测试的 `--recover-cache` flag：只有用户执行 `hopper-dispatch --probe <vendor> --recover-cache` 时，工具才先创建受控本地 backup、保留原文件供诊断/回滚，再将 fresh v1 cache atomic replace 到 active path；普通 dispatch/probe 永不隐式启动此流程。若现有 probe 层不能承载 flag，本阶段实现范围必须增加同语义的 `hopper-dispatch --recover-cache <vendor>` command 和等价测试，不能仅保留文字约定。恢复失败仍为 `unknown` 并保留原文件/backup，不把失败当作 missing。
...
- dashboard rollout 必须先发布 null-safe client，再启用 feature-gated 过渡 server：该 server 只能返回安全、shape-compatible shim（`notes: []`、`cacheError: null`、`modelsSource: null`，`binaryPath: null` 或 omit，且绝不含 raw 值）和 v2 safe enums。compatibility window 期间 old SPA 对 missing/null/unknown legacy field 只能显示 unavailable，不能崩卡；new client 忽略 old-server raw legacy fields。窗口结束、确认 safe client 覆盖后才删除 legacy keys。`inventoryContractVersion` 只作为 compatibility diagnostic，缺失、未知或未来值不得成为 hard assertion/throw。外部 consumer 的 deprecation notice 必须说明 v1 raw fields 已删除及安全替代字段。rollback 只能回到 safe shim、关闭 card rendering 或回滚到仍不暴露 raw field 的 client/server bundle；不得以恢复任何 legacy raw field 作为回滚手段。
...
| Claude/OpenCode zero-spawn selector metadata | schema v1 cache envelope/manifest、vendor-adapter-catalog binding、validity/time/version、literal selector、suffix literal、closed grammar union、unknown forward field；fixtures 覆盖 `fable`、`sonnet`、`best`、`default`、`opusplan`、`[1m]`、未声明 `[N_unit]` | dispatch 不 spawn/probe/network；只对 post-policy effective selector 用有效 envelope 分类，且不建立 alias→actual 映射；unsupported schema/adapter mismatch/过期/缺失/malformed/未枚举为 `selector_kind=unknown`、`unverified`，绝不 false `mismatch`/`alias-resolved`。 |
...
| Scalar safety | models 包含引号、反斜杠、换行、control character、`:`、`#`、`---`、`null` 等值 | YAML quoted scalar round-trip 后才 JSON parse；禁止 injection/plain scalar；仅 `Array<string>` 可进入 observed models，`null`/object/非 string member 一律丢弃；JSONL 仍为 array。 |
...
| Sync/background finalization | 同一 parsed result 分别经 `--write` 同步路径与后台 runner；append failure、frontmatter failure、crash/re-entry、已有 terminal event、orphan reaper late-reread race | 两路径调用同一 finalizer/同一 event-first 顺序；从 0 到恰好 1 条 terminal JSONL event，成功后同字段集/同 resolution；append failure 不写 terminal frontmatter，frontmatter failure 显示 finalizing；reaper 只在唯一匹配 event 且 late reread inputs 未变时做 frontmatter-only idempotent repair；所有 no-op cases 不得重试 append 或产生第二 terminal event。 |
| Optional-event allowlist | 终态 event 带所有新 attestation fields | `OPTIONAL_EVENT_FIELDS` 保留每一字段，JSONL round-trip 不丢值。 |
| Cache/probe semantics | missing cache、旧 v1 entry、含 root/vendor/nested unknown future field 的 v1 entry、version mismatch、malformed cache、`--probe <vendor> --recover-cache` success/failure、probe/capability/runtime diagnostics priority | diagnostics-aware reader 区分 missing/ok-v1/mismatch/malformed；仅 missing 初始化、v1 owned fields additive merge 且所有 unknown field 保留；mismatch/malformed 为 unknown、普通 writer no-write，version mismatch 原文件字节不变；仅 explicit recovery 可 backup 后 atomic replace fresh-v1，失败不降格为 missing；每个 trigger 选 canonical scoped diagnostic code。 |
...
| Rendering/privacy/dashboard migration | `--models`、`--setup`、`--capabilities`、`--check-model`、result/progress、dashboard safe-shim/v2 fixture、old/new server-client matrix、six-field future-enum 与 rollback fixture | 六个 v2 public field 各自断言 closed/unknown(nullable basename)；每个 public surface 的 `source_label`/`diagnosticCode` 都断言为 canonical closed enum，unknown/future/raw 值归一 unknown；API response 的 recursive closed-set key/value scan 在每一嵌套 depth 断言不存在 `sourceNote`、`binaryPath`（仅 shim null/omit）、`notes`、`cacheError`、`modelsSource`、absolute path、URL、home path、PATH、stderr、auth/provider 或 secret-like content。先 null-safe client、再 safe shim、compatibility window 后删 key；old client 不崩卡，新 client 忽略 old-server raw legacy field，`inventoryContractVersion` 仅 diagnostic；rollback bundle 只回到 safe shim/disable card，仍不重现 raw fields。 |
