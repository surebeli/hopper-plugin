...
所有公共 surface（`--capabilities`、`--models`、`--setup`、`--check-model`、dashboard/API、`--result` 与 `--progress`）使用同一 closed allowlist：`source_label` 只能是上述六个值，未知/future/raw 值一律归一为 `unknown`；`diagnosticCode` 只能是 `none|probe-failed|catalog-unavailable|metadata-cache-missing|metadata-cache-stale|metadata-cache-version-unsupported|cache-version-unsupported|cache-malformed|runtime-model-metadata-absent|unknown`，任何未映射错误为 `unknown`。UI 可在本地把 enum 映射为文案，但不得让原始字符串进入响应或 handoff。`source_label` 不是绝对文件路径、URL、完整 PATH 或原始 stderr，也不得直接复用 `models_source`。`binary_basename` 必须取已经解析的 command basename，且不能从未验证的自由文本直接回显。

probe/能力失败必须先映射到 closed `diagnosticCode`：已知 probe spawn/exit/parse failure 为 `probe-failed`，无 catalog 为 `catalog-unavailable`，metadata cache 缺失/过期/版本不兼容分别为 `metadata-cache-missing`/`metadata-cache-stale`/`metadata-cache-version-unsupported`，cache parse failure 为 `cache-malformed`；无法可靠分类的失败为 `unknown`。`notes`、`sourceNote`、`cacheError` 与 `modelsSource` 都不是 public field。需要保留 raw probe/能力说明时，它只可留在私有、受控的本地诊断日志，且不得复制进 cache `notes`、frontmatter、JSONL、dashboard/API 或任何 CLI summary；原始 stderr、auth/login 文案、provider 名称、parse exception、config/binary path、URL、environment value 和 credential 均同样禁止。
...
adapter 必须在其 model capability metadata 中为实际传入 `--model` 的 selector 进行分类，而不是在 dispatcher 中根据字符串格式推测。该 metadata 只分类，实际 comparison 仍完全由 `model-normalize.js` 执行。分类本身是 zero-spawn：dispatch 必须按以下优先级仅读取 (1) 已验证、已 sanitize、版本兼容且未过期的 capabilities cache，或 (2) adapter 发布的已版本化 selector metadata；两者都不可用时为 `unknown`。dispatch 不得为分类额外执行 CLI、probe 或网络请求。metadata 仅可枚举 literal selector 或显式 variant/identity-grammar entry（例如已声明的 OpenCode `provider/model` variant）；不得用正则、字符串形状或 alias-to-actual-ID 硬编码推断。cache 缺失、过期、版本不兼容、schema 不可读或 selector 未枚举时，显式 selector 统一为 `unknown`，resolution 为 `unverified`，不得产生 `mismatch` 或 `alias-resolved`。
...
只有“恰好一条、task identity 匹配且可解析”的 orphan terminal JSONL event 可触发 orphan reaper。它只能执行 **frontmatter-only** 的 idempotent repair：从该 event 重建 canonical terminal frontmatter 并写入 `terminal_event_emitted=true`，绝不得 append、重放或合成第二条 terminal JSONL event。0 条、超过 1 条、task ID 不匹配、event 损坏或已存在完整 terminal frontmatter 时，reaper 不写入并由 reader 保持既有 in-progress/partial/conflict precedence；完整 frontmatter 始终优先于 orphan event。repair 重试必须比较 canonical frontmatter 是否已完整，完成后为 no-op。
...
对未来真正的 cache version mismatch，normalizer、dispatch、result 与 setup 不采用任何 cached model evidence，统一降级显示 `cache_schema=unknown`/`catalog=unknown`；普通 `setVendorCache` 也不得向该文件写入、自动删除、重建或覆盖旧文件，且必须保留原 cache 文件的字节内容不变。`--models` 可以返回受控的 `cache-version-unsupported` 诊断，但也不能泄露 cache absolute path。只有一个专门的、显式设计过的恢复流程才可替换不兼容或 malformed cache：操作员显式选择 recovery，工具先保留原文件供受控诊断/回滚，再原子生成 fresh v1 cache；普通 dispatch/probe 永不隐式启动此流程。恢复失败仍为 `unknown` 并保留原文件，不把失败当作 missing。
...
- dashboard rollout 必须先发布 null-safe client，再启用 feature-gated 过渡 server：该 server 只能返回安全、shape-compatible shim（例如 `notes: []`、`cacheError: null`、`modelsSource: null`，且不含 raw 值）和 v2 safe enums。compatibility window 期间 old SPA 对 missing/null/unknown legacy field 只能显示 unavailable，不能崩卡；new client 忽略 old-server raw legacy fields。窗口结束、确认 safe client 覆盖后才删除 legacy keys。`inventoryContractVersion` 只作为 compatibility diagnostic，缺失、未知或未来值不得成为 hard assertion/throw。外部 consumer 的 deprecation notice 必须说明 v1 raw fields 已删除及安全替代字段。rollback 只能回到 safe shim、关闭 card rendering 或回滚到仍不暴露 raw field 的 client/server bundle；不得以恢复任何 legacy raw field 作为回滚手段。
...
| Claude/OpenCode zero-spawn selector metadata | versioned sanitized capabilities cache、adapter-published versioned literal/variant metadata、cache 缺失/过期/版本不兼容/schema 坏、未枚举 alias；fixtures 覆盖 `fable`、`sonnet`、`best`、`default`、`opusplan`、`[1m]` | dispatch 不 spawn/probe/network；仅有效 metadata 分类，且不建立 alias→actual 映射；缺失/过期/不兼容/未枚举为 `selector_kind=unknown`、`unverified`，绝不 false `mismatch`/`alias-resolved`。 |
...
| Rendering/privacy/dashboard migration | `--models`、`--setup`、`--capabilities`、`--check-model`、result/progress、dashboard safe-shim/v2 fixture、old/new server-client matrix、future enum 与 rollback fixture | 每个 public surface 的 `source_label`/`diagnosticCode` 都断言为 closed enum，unknown/future/raw 值归一 unknown；API response 的 recursive closed-set key/value scan 在每一嵌套 depth 断言不存在 `sourceNote`、`binaryPath`、`notes`、`cacheError`、`modelsSource`、absolute path、URL、home path、PATH、stderr、auth/provider 或 secret-like content。先 null-safe client、再 safe shim、compatibility window 后删 key；old client 不崩卡，新 client 忽略 old-server raw legacy field，`inventoryContractVersion` 仅 diagnostic；rollback bundle 只回到 safe shim/disable card，仍不重现 raw fields。 |
